...
class InferenceApi {
  ...
  ///
  /// Vision query for the object detection task.
  /// This task reads some image input and outputs the likelihood of classes &
  ...
  /// The model to use for the task
  ///
  Future<List<ApiResponseVisionObjectDetection?>?> queryVisionObjectDetection(
      {required Uint8List imageFile, required String model}) async {
    final taskParameters = {'mediaFile': imageFile};
    final response = await _withHttpInfo(taskParameters, model);
    ...
    if (response.body.isNotEmpty &&
      ...
      final responseBody = await _decodeBodyBytes(response);
      return (await apiClient.deserializeAsync(
          responseBody, 'List<QueryVisionObjectDetectionTask>'));
    }
    return null;
  }
  ...
  ///
  /// Vision query for the image segmentation task.
  /// This task reads some image input and outputs the likelihood of classes &
  /// bounding boxes of detected objects.
  ///
  /// [taskParameters]
  ...
  /// The model to use for the task
  ///
  Future<List<ApiResponseVisionImageSegmentation?>?>
      queryVisionImageSegmentation(
          {required Uint8List imageFile, required String model}) async {
    final taskParameters = {'mediaFile': imageFile};
    final response = await _withHttpInfo(taskParameters, model);
    ...
    if (response.body.isNotEmpty &&
      ...
      final responseBody = await _decodeBodyBytes(response);
      return (await apiClient.deserializeAsync(
          responseBody, 'List<QueryVisionImageSegmentationTask>'));
    }
    return null;
  }
