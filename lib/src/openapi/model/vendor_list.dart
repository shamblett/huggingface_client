...
class VendorList {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is VendorList &&
     other.items == items;
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "VendorList[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "VendorList[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<VendorList> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<VendorList>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<VendorList>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = VendorList.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
