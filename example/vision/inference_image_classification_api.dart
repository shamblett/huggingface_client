...
void main() async {
  ...
  //
  // Inference Vision Image Classification task
  //

  // Assumes you are running the example from the project root i.e
  // 'dart example/vision/inference_image_classification_api.dart'
  final imageFilePath =
  ...
  print('');
  print('*** Inference API Vision Image Classification task ***');
  print('');
  try {
    final result = await apiInstance.queryVisionImageClassification(
        imageFile: imageFileContents, model: 'google/vit-base-patch16-224');
    if (result!.isNotEmpty) {
      for (final row in result) {
        print(row);
      }
    } else {
      print('Inference Vision Image Classification result is empty');
    }
  } catch (e) {
    print(
        'Exception when calling Vision Inference Image Classification API : $e - exiting');
    return;
  }
